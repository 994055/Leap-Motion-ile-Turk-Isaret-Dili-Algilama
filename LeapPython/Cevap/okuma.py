...
try:
                ...
                for hand in frame.hands:
                    ...
                    if len(frame.hands) == 2 :
                        ...
                        if handType == "Right hand":
                            ...
                            liste=( sag_posx, sag_posy, sag_posz, sag_pitch, sag_roll, sag_yaw, sag_armdx, sag_armdy,
                            ...
                            SampleListener.okuListeRight.append(liste)
                            print ( "*********Kayit Eklendi.************")
                        if handType=="Left hand":
                            ...
                            liste=(sol_posx, sol_posy, sol_posz, sol_pitch, sol_roll, sol_yaw, sol_armdx, sol_armdy,
                            ...
                            liste=np.array(liste)
                            SampleListener.okuListeLeft.append(liste)
                            print ("*********Kayit Eklendi.************")
    ...
    def main():
        ...
        try:
            sys.stdin.readline()
            print (SampleListener.okuListeLeft)
            datasetX = []
            listeLeft = SampleListener.okuListeLeft
            listeRight = SampleListener.okuListeRight
            if len(listeLeft)==0:
            ...
            if len(listeRight)==0:
                for i in range(0,390):
                    listeRight.append(0)
            listeLeft=np.array(listeLeft)
            listeRight=np.array(listeRight)
            print(listeLeft.shape[1])
            for i in range(0, listeLeft.shape[0]):
                liste = []
                for j in range(0, int(listeLeft.shape[1])):
                    liste.append(listeLeft[i][j])
                for k in range(0, int(listeRight.shape[1])):
                    liste.append(listeRight[i][k])
                datasetX.append(liste)
            print(datasetX)
            np.save("../dataX",datasetX)
            datasetX=np.array(datasetX)
            print(datasetX.shape)
            """filename = open("../RandomForest.pkl",'rb')
            model = joblib.load(filename)
            result=model.predict(datasetX)
            metin = ""
            sayac=0
